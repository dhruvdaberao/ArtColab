...
export function GuestDisplayNameModal({
  ...
  description = 'Enter the name you want to use in this Froddle session before joining or creating a room.',
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/35 px-4 py-6 backdrop-blur-sm">
      <form onSubmit={submit} className="w-full max-w-md space-y-4 rounded-[2rem] border-2 border-black bg-[#fffdf7] p-5 shadow-[8px_8px_0_rgba(17,24,39,0.08)] sm:p-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
          ...
          <Button type="submit" disabled={submitting} className="w-full sm:w-auto bg-[color:var(--brand-green)] hover:bg-[color:var(--brand-green-strong)]">{submitting ? 'Saving…' : confirmLabel}</Button>
        </div>
      </form>
    </div>
  );
