...
export function useRoomSocket(roomId: string, userId: string, displayName: string, avatarUrl?: string) {
  ...
  useEffect(() => {
    ...
    socket.connect(); setStatus('connecting'); setError(null); setHasJoined(false);
    ...
    const applyRoom = (room: RoomState) => { setParticipants(room.participants); setStrokes(room.strokes); setChatMessages(room.chatMessages ?? []); setMode(room.mode ?? 'free-draw'); };
    ...
    socket.on(SOCKET_EVENTS.STROKE_EVENT, (event) => { if (event.type === SOCKET_EVENTS.STROKE_START) setStrokes((prev) => [...prev, event.stroke]); if (event.type === SOCKET_EVENTS.STROKE_APPEND) setStrokes((prev) => prev.map((stroke) => stroke.strokeId === event.strokeId ? { ...stroke, points: [...stroke.points, ...event.points] } : stroke)); });
    ...
    socket.on(SOCKET_EVENTS.STROKE_UNDONE, ({ strokeId }: { strokeId: string }) => setStrokes((prev) => prev.filter((stroke) => stroke.strokeId !== strokeId)));
