...
export function AuthCard({ initialView = 'login', redirectTo = '/' }: { initialView?: AuthView; redirectTo?: string }) {
  ...
  return <Card className="space-y-4 bg-[color:var(--surface)] p-6">
    <div className="space-y-1"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[color:var(--text-muted)]">Account access</p><h1 className="text-3xl font-black text-[color:var(--text-main)]">Art Colab</h1><p className="text-sm text-[color:var(--text-muted)]">Sign in or create an account to save your identity across rooms.</p></div>
    <div className="flex gap-2 rounded-[1.25rem] border-2 border-[color:var(--border)] bg-[color:var(--surface-soft)] p-1">
      <button type="button" onClick={() => { setError(null); setView('login'); }} className={`flex-1 rounded-2xl px-3 py-2 text-sm font-semibold transition ${view === 'login' || view === 'forgot-request' || view === 'forgot-verify' ? 'bg-[color:var(--primary)] text-[color:var(--surface)]' : 'text-[color:var(--text-main)]'}`}>Login</button>
      <button type="button" onClick={() => { setError(null); setView('register'); }} className={`flex-1 rounded-2xl px-3 py-2 text-sm font-semibold transition ${view === 'register' ? 'bg-[color:var(--primary)] text-[color:var(--surface)]' : 'text-[color:var(--text-main)]'}`}>Create account</button>
    </div>
...
function LoginForm({ onSubmit, onForgot, setError }: { onSubmit: (i: string, p: string) => Promise<void>; onForgot: () => void; setError: (v: string | null) => void }) {
  ...
  return <form className="space-y-3" onSubmit={(e)=>{e.preventDefault(); setError(null); setSubmitting(true); onSubmit(identifier.trim(), password).catch((err)=>setError((err as Error).message)).finally(()=>setSubmitting(false));}}>
    ...
    <button type="button" onClick={onForgot} className="text-sm font-medium text-[color:var(--text-main)] underline">Forgot password?</button>
  </form>;
}
