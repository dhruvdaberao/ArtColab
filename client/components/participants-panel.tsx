...
export function ParticipantsPanel({ participants, userId }: ParticipantsPanelProps) {
  return (
    <aside className="w-full rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm lg:w-[300px]">
      ...
      {participants.length === 0 ? (
        ...
        <ul className="space-y-2">
          {participants.map((participant) => {
            ...
            return (
              <li key={participant.socketId} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-slate-700">{participant.displayName}</p>
                  <p className="text-xs text-slate-500">{isYou ? 'You' : 'Connected'}</p>
                </div>
              </li>
            );
          })}
