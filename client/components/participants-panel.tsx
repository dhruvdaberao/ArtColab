...
export function ParticipantsPanel({ participants, userId, compact = false }: ParticipantsPanelProps) {
  return (
    <aside className={`w-full rounded-[1.75rem] border-2 border-[color:var(--border)] bg-[color:var(--surface)] shadow-[var(--shadow)] transition ${compact ? "p-3" : "p-4 2xl:sticky 2xl:top-6"}`} aria-label="Participants panel">
      ...
      {participants.length === 0 ? (
        ...
        <ul className={`space-y-2.5 ${compact ? "max-h-72 overflow-y-auto pr-1" : ""}`}>
